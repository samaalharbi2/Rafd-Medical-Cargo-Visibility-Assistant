...
"ar": {
 ...
 "about_solution": "محرّك استرجاع يعتمد على حقول البيانات (Metadata-aware) يعطي إجابات دقيقة وقابلة للتحقق، مع طبقة صياغة ذكية اختيارية لا تغيّر أي رقم أو معرّف.",
...
"en": {
 ...
 "about_solution": "A metadata-aware retrieval engine gives exact, verifiable answers, with an optional AI phrasing layer that never changes a number or identifier.",
...
def _css(ar, dark):
    ...
    al = "right" if ar else "left"
    side = "right" if ar else "left"
    st.markdown(f"""
<style>
.stApp {{ background:{bg}; color:{text}; }}
[data-testid="stSidebar"] {{ background:{card}; border-{ 'left' if ar else 'right'}:1px solid {border}; }}
.block-container {{ direction:{d}; text-align:{al}; padding-top:0.8rem; max-width:1080px; }}
...
.kpi .n {{ color:{navy if not dark else '#E6EDF3'}; font-size:1.35rem; font-weight:700; line-height:1; }}
.kpi .l {{ color:{teal}; font-size:.7rem; margin-top:4px; }}
.intro {{ background:{card}; border:1px solid {border}; border-radius:10px;
  padding:12px 16px; color:{muted}; font-size:.92rem; margin-bottom:8px; }}
[data-testid="stChatMessageContent"] {{ direction:{d}; text-align:{al}; }}
...
h4 {{ color:{navy if not dark else '#cfe3ff'}; }}
