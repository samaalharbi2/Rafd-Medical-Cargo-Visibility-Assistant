"""Arabic display translations for Rafd (airports, facilities, events)."""

AIRPORT_AR = {
    "JED": "جدة", "RUH": "الرياض", "DMM": "الدمام", "MED": "المدينة المنورة",
}
...
EVENT_AR = {
    ...
    "Arrived Destination": "وصلت مطار الوجهة", "SFDA Hold": "معلّقة لدى هيئة الغذاء والدواء",
    "SFDA Cleared": "تم التخليص من هيئة الغذاء والدواء", "Cold Chain Alert": "تنبيه سلسلة التبريد",
    "Delivered": "تم التسليم", "Delayed": "تأخير", "Cancelled": "ملغاة",
}

def ap(code, ar):
    """Airport: Arabic name for Saudi airports, else keep the code."""
    return AIRPORT_AR.get(code, code) if ar else code

def fac(name, ar):
    return FACILITY_AR.get(name, name) if ar else name

def evt(name, ar):
    return EVENT_AR.get(name, name) if ar else name
